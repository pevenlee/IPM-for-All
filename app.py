    ...
    with st.chat_message("assistant"):
        ...
        # 3. 深度分析 (修复无数据问题)
            ...
            with st.spinner(f"🧠 专家拆解分析思路 ({MODEL_SMART})..."):
                ...
                【重要指令 - 避免空数据】
                1. **列名校验**: 如果用户说的 "ATC2" 在表中不存在，请寻找最相似的列（如 "ATC-II", "治疗大类"）。请仔细查看上下文中的【所有列名】。
                2. **时间校验**: 先检测数据中包含哪些年份。如果只有一年数据，**不要**计算同比增长（YoY），改为计算当期绝对值或份额。
